 ...
 * @Author: JackChou
 * @LastEditTime: 2021-01-17 20:55:40 +0800
      ...
      <div class="panel-body remove-padding-horizontal">
        ...
        <ul class="list-group row topic-list">
          <li class="list-group-item" v-for="(article, index) in articles" :key="index">
            <router-link
              :to="`/acticles/${article.articleId}/content`"
              tag="div"
            ...
            <router-link :to="`/${article.uname}`" tag="div" class="avatar pull-left">
              <img :src="article.uavatar" alt="头像" class="media-objet img-thumbnail avatar avatar-middle" />
            </router-link>
...
export default {
  ...
  data() {
    return {
      ...
      filter: 'default',
      /**
       * 列表数据添加到数据对象，而不是直接写在模板里，这样做不仅可以简化模板，还可以方便更改逻
       * 辑。
       */
      filters: [
  ...
  */
  ...
  watch: {
    ...
    /**
     * 当路由从 /topics?filter=default 跳到类似 /topics?filter=vote 的地址时，Home.
     * vue 会被复用，beforeRouteEnter 不会被再次调用，此时，我们需要监听 '$route' 以根据
     * 路由参数的变化，重新获取对应数据
     */
    $route(to) {
